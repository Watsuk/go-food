...
CREATE TABLE `trucks` (
  ...
  CONSTRAINT `truck_user_id` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `orders` (
  ...
  `hours` time NOT NULL,
  `status` enum('pending','accepted','rejected','completed', 'handedover') NOT NULL DEFAULT 'pending',
...
CREATE TABLE `tokens` (
  ...
  CONSTRAINT `tokens_user_id` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping buffet for table `tokens`
--
